...
RSpec.describe QuestionsController, type: :controller do
  ...
  describe "POST create" do
    it "increases the number of Questions by 1" do
      expect { post :create, {question: {title: "Title", body: "Body", resolved: true}} }.to change(Question, :count).by(1)
    end
  ...
  describe "PUT update" do
    it "updates question with expected attributes" do
      ...
      update_questions = assigns(:question)
      expect(update_questions.id).to eq my_question.id
      expect(update_questions.title).to eq new_title
      expect(update_questions.body).to eq new_body
    end
